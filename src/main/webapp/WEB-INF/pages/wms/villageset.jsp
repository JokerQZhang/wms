...
.tree li.parent_li>span:hover, .tree li.parent_li>span:hover+ul li span {
	...
	function modifyCun(){
		...
		ajaxLoadDaialog({url:'<c:url value='/wmsd/editPartyGroupCun'/>',title:'<fmt:message key="partyGroupDetail.heading"/>',width:600,height:500,beforeDialogOpen:undefined,afterDialogOpen:afterPartyGroupFormOpen,data:{isNowXiang:isNowXiang,partyId:selectedGroupId,method:'Modify',from:'list'}});
	}
	function addPeople(){
		if(selectedGroupId==""){
			alert("请先选择需要修改的村信息!");
		...
		ajaxLoadDaialog({url:'<c:url value='/wmsd/editCunPerson'/>',title:'<fmt:message key="partyGroupDetail.heading"/>',width:600,height:500,beforeDialogOpen:undefined,afterDialogOpen:afterPartyGroupFormOpen,data:{belongGroupId:selectedGroupId,isNowXiang:isNowXiang,method:'Add',from:'list'}});
	}
	function villageTreeSelect(partyId, isXiang){
		loadingEffect($(".col-sm-7"));
		$.ajax({
			url:"<c:url value='/wmsd/cunInfoDtl'/>"
			,type:"post"
