		...
		<tbody>
			<c:forEach items="${persons}" var="person">
				<tr onclick="commonTableListSelect(this,afterSelectPerson)">
					<td><a class="btn btn-info btn-sm" href="#" onclick="ajaxLoadDaialog({url:'<c:url value='/wmsd/editPositivePerson'/>',title:'<fmt:message key="personDetail.heading"/>',width:1100,height:500,beforeDialogOpen:undefined,afterDialogOpen:afterPersonFormOpen,data:{personId:'${person.personId}',from:'list'}})">${person.personId}</a></td>
					<td><a class="btn btn-info btn-sm" href="#" onclick="ajaxLoadDaialog({url:'<c:url value='/wmsd/approvalDangPerson'/>',title:'发展党员信息',width:500,height:350,beforeDialogOpen:undefined,afterDialogOpen:undefined,data:{partyId:'${person.partyId}',personId:'${person.personId}',from:'list'}})">${person.name}</a></td>
					<td>${person.cardId}</td>
					<td>${person.gender}</td>
					<td>${person.phone}</td>
					<td>
