		...
		<tbody>
			<c:forEach items="${persons}" var="person">
				<tr onclick="commonTableListSelect(this,afterSelectPerson)">
					<td><a class="btn btn-info btn-sm" href="#" onclick="ajaxLoadDaialog({url:'<c:url value='/wmsd/editDangPerson'/>',title:'<fmt:message key="personDetail.heading"/>',width:1100,height:500,beforeDialogOpen:undefined,afterDialogOpen:afterPersonFormOpen,data:{personId:'${person.personId}',from:'list'}})">${person.personId}</a></td>
					<td><a class="btn btn-info btn-sm" href="#" onclick="window.open('<c:url value='/wmsd/cpDtlInfo?personId=${person.personId}'/>');">${person.name}</a></td>
					<td>${person.cardId}</td>
					<td>${person.gender}</td>
					<td>${person.phone}</td>
				</tr>
